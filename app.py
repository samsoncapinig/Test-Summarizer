...
if uploaded_files:
    ...
    if "Total points" not in combined_df.columns:
        ...
        summary["Percentage Passing"] = (
            summary["Passers"] / summary["Total_Takers"] * 100
        )
        ...
        display_summary["Percentage Passing"] = (
            display_summary["Percentage Passing"].round(2).astype(str) + "%"
        )
        ...
        fig3 = go.Figure(data=[
            go.Pie(
                ...
                marker=dict(colors=["#2E8B57", "#DC3545"])
            )
        ])
